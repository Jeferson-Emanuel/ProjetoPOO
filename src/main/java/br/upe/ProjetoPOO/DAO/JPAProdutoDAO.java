...
public class JPAProdutoDAO implements ProdutoDAO {
	...
	public Produto obterPorProduto(String produto) {
		Produto produtoTemp = null;

		em.getTransaction().begin();

		try{			
			produtoTemp = em.createQuery(
					"SELECT u from Produto u WHERE u.produto = :produto", Produto.class).
	...
	public void /*Produto*/remove(int id) {
		em.getTransaction().begin();
		Produto produto = em.find(Produto.class, id);
		System.out.println("Excluindo dados de: " + produto.getProduto());
		em.remove(produto);
		em.getTransaction().commit();
		em.close();
		//return produto;
